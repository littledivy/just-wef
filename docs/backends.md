...
| --------------------- | ------------- | ------------- | ------- | --------- |
| [CEF](../cef)         | Chromium 144  | multi-process | yes     | yes       |
| [WebView](../webview) | system native | single        | no      | yes       |
| [Winit](../winit)     | none          | single        | n/a     | no        |
...
Sources live in [`cef/`](../cef); shared native features come from
`backend-common`. On Windows the backend links the static CRT (`/MT`), so
everything it links — including `backend-common` — is built `/MT`.
...
Sources live in [`webview/`](../webview), one file per platform
(`webview_macos.mm`, `webview_windows.cc`, `webview_linux.cc`), sharing
`backend-common` for menus, tray, dialogs, dock, and notifications.
...
handles needed to create a rendering surface. Sources in [`winit/`](../winit).
...
dialogs, notifications, key mapping) in [`backend-common/`](../backend-common),
