...
wef stops at the backend and the runtime. A build produces a backend executable
and your runtime shared library; turning that into something you can ship — and
keeping it up to date — is the responsibility of the embedder that wraps wef,
not of wef itself.

## Bundling

wef does not produce an application bundle. The embedder decides how the backend
...
design — wef stays unopinionated so that a host such as the
[`deno desktop`](https://github.com/denoland/deno) tooling can own the packaging
format end to end.

One detail does reach into wef on macOS. Several features —
...
features come to life only once the embedder has bundled the app. wef hard-codes
no bundle identifier of its own, leaving the embedder free to set its own
identity.

## Code signing & notarization

wef does not sign or notarize anything. Code signing (macOS Developer ID +
notarization, Windows Authenticode) is applied by the embedder to the final
bundle, using its own certificates and entitlements. Because wef carries no
...
the embedder rather than wef.

## Updates

wef has no built-in updater. Shipping new versions — full replacement or
...
uses applies without special support from wef.
